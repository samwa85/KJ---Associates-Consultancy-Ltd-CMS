...
// Initialize from unified config (API_CONFIG) with legacy support for meta tags
(function () {
  ...
  // Priority: 1. API_CONFIG, 2. Global overrides, 3. Meta tags, 4. Hardcoded fallback
  SUPABASE_CONFIG.url = window.API_CONFIG?.supabaseUrl ||
    window.SUPABASE_URL ||
    metaUrl ||
    'https://supabasekong-j8k8sksckccs4ccogsscccww.31.97.79.197.sslip.io';

  SUPABASE_CONFIG.anonKey = window.API_CONFIG?.supabaseKey ||
    window.SUPABASE_ANON_KEY ||
    metaKey ||
    '';

  console.log('[SupabaseClient] Initialized with URL:', SUPABASE_CONFIG.url);
})();
